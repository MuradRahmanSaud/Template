...
// API: Fetch Google Sheet CSV data with cache busting
app.get("/api/sheet-data", async (req, res) => {
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        success: false,
        error: `Google Sheet fetch failed with status ${response.status}: ${response.statusText}. Please ensure the sheet sharing is set to 'Anyone with the link can view'.`,
      });
    }
